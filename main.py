...
def get_full_route(data, manager, routing, solution, df_drivers):
    ...
    for vehicle_id in range(data["num_vehicles"]):
        ...
        answer.append(elem)
    pd.DataFrame(all_routes).dropna(axis=1).to_json(
        "./output/route_points.json", orient="records"
    )
    ...
    for route in all_routes:
        ...
        for coord in route:
            if type(coord) != float:
                c = {"lat": eval(coord)[0], "lng": eval(coord)[1]}
                ruta.append(c)
        rutas.append(ruta)
...
def main():
    ...
    if solution:
        # answer = get_full_route_2(data, manager, routing, solution, df_drivers)
